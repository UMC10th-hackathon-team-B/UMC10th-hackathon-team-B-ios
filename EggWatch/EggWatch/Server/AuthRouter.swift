...
enum AuthRouter {
    ...
    case refreshToken(refreshToken: String)  // 토큰 재발급 API
    case logout(refreshToken: String)        // 로그아웃 API
}
...
extension AuthRouter: TargetType {  // Moya에게 AuthRouter가 API라우터라고 알려줌.
    ...
    var path: String {
        ...
        case .refreshToken: return "api/v1/auth-tokens"           // 토큰 재발급 경로
        case .logout:       return "api/v1/auth-sessions/current" // 로그아웃 경로
        }
    }
    ...
    var method: Moya.Method {   // HTTP요청 방식. 둘 다 데이터를 보내는 POST 방식.
        ...
        case .refreshToken: return .post    // 토큰 재발급은 POST
        case .logout:       return .delete  // 로그아웃은 DELETE
        }
    }
    ...
    var task: Task {    // 백엔드에 실제로 보낼 데이터를 담는 곳
        ...
        case .refreshToken(let refreshToken):
            return .requestJSONEncodable(TokenRefreshRequest(refreshToken: refreshToken))  // 리프레시 토큰 전송
        case .logout(let refreshToken):
            return .requestJSONEncodable(LogoutRequest(refreshToken: refreshToken))  // 리프레시 토큰 전송
        }
    }
